...
class AWSSNSTests: XCTestCase {
    ...
    func testDeletePlatformEndpoint() {
        ...
        snsClient?.createPlatformEndpoint(token: token, platformApplicationArn: "arn:aws:sns:us-west-2:487164526243:app/APNS_SANDBOX/Test") { _, endpointArn, error in
            self.snsClient?.deleteEndpoint(endpointArn: endpointArn!) { _, error in
